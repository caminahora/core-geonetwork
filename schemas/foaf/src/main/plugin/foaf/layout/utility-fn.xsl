...
<xsl:stylesheet version="2.0"
  ...
  xmlns:gn-fn-foaf="http://geonetwork-opensource.org/xsl/functions/profiles/foaf"
  xmlns:gn-fn-metadata="http://geonetwork-opensource.org/xsl/functions/metadata"
  exclude-result-prefixes="#all">

  <!-- return the lang iso3code in uper case. -->
  <xsl:function name="gn-fn-foaf:getLangId" as="xs:string">
    <xsl:param name="md" />
    <xsl:param name="lang" />
    <xsl:value-of select="concat('#', upper-case($lang))" />
  </xsl:function>

  <xsl:function name="gn-fn-foaf:getCodeListType" as="xs:string">
